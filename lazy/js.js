...
function generateMenu(items) {
	document.querySelectorAll(items).forEach(function (item) {
		item.addEventListener("mouseenter", function () {
			let spaceLeft = item.offsetLeft + item.offsetWidth + item.querySelector("ul.sub-menu").offsetWidth;
			if (typeof item.parentElement.getAttribute("id") === "undefined") {
				if (spaceLeft > window.innerWidth) {
					item.querySelector("ul.sub-menu").style.marginTop = "-40px";
					item.querySelector("ul.sub-menu").style.marginLeft = "-" + item.querySelector("ul.sub-menu").offsetWidth + "px";
					item.querySelector("ul.sub-menu").style.transition = "none";
					item.querySelector("ul.sub-menu").style.display = "block";
				} else {
					item.querySelector("ul.sub-menu").style.marginLeft = item.offsetWidth + "px";
					item.querySelector("ul.sub-menu").style.transition = "none";
					item.querySelector("ul.sub-menu").style.display = "block";
				}
			} else {
				if (spaceLeft > window.innerWidth) {
					item.querySelector("ul.sub-menu").style.marginLeft = "-" + (spaceLeft - window.innerWidth + 30) + "px";
					item.querySelector("ul.sub-menu").style.transition = "none";
					item.querySelector("ul.sub-menu").style.display = "block";
				} else {
					item.querySelector("ul.sub-menu").style.transition = "none";
