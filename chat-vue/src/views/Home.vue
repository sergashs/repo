	...
	<div class="chat-page">
		<div class="chat">
			...
			<div class="add-message">
				<div class="row g-0">
					<div class="col-10">
						<input v-if="!form.nameIsSet" class="form-control mb-3" type="text" placeholder="nickName" v-model="form.name" />
						<input class="form-control" type="text" placeholder="Type a Message" v-model="form.message" @keyup.enter="sendMessage" />
					</div>
					<div class="col-2 d-flex align-items-end justify-content-end">
						<button class="btn-send" @click="sendMessage">
							<svg width="158px" height="158px" viewBox="0 -12 158 158" fill="none" xmlns="http://www.w3.org/2000/svg">
...
export default {
	...
	created() {
		// axios.delete(`https://posts-3103b-default-rtdb.firebaseio.com/posts.json?`, {});
		...
		if (userName) {
			this.form.nameIsSet = true;
			this.form.name = userName;
		}
	},
	mounted() {
		setTimeout(() => {
			var objDiv = document.querySelector(".messages-holder");
			objDiv.scrollTop = objDiv.scrollHeight;
		}, 300);
	},
	methods: {
		async getMessages() {
			axios.get("https://posts-3103b-default-rtdb.firebaseio.com/posts.json?").then((response) => (this.messages = response.data));
		},
		sendMessage() {
			if (this.form.name && this.form.message) {
				axios.post(`https://posts-3103b-default-rtdb.firebaseio.com/posts.json?`, { userName: this.form.name, message: this.form.message }).then((response) => {
